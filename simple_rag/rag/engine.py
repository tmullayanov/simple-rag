...
def build_answers_retriever(vector_store: SimpleVectorStore):
    ...
    def get_answers(state: RagState):
        answers = [i for d in state['questions'] for i in vector_store.lookup_answers(d.page_content)]
...
def build_summarizer(llm: BaseChatModel):
    def answer_based_on_context(state: RagState):
        ...
        messages = rag_prompt.invoke({"questions": state["questions"], "answers": context, "raw_input": state['raw_input']})
...
def build_rag_graph(
    ...
    graph_builder = StateGraph(RagState).add_sequence([manual_retrieve, get_answers, answer_based_on_context])
